...
func (s *TodoService) CreateTodo(ctx echo.Context, userID string, payload *todo.CreateTodoPayload) (*todo.Todo, error) {
	...
	if payload.ParentTodoID != nil {
		...
		if !parentTodo.CanHaveChildren() {
			err := errs.NewBadRequestError("parent todo cannot have children (subtasks can't have sbtasks)", false, nil, nil, nil)
...
func (s *TodoService) UploadTodoAttachment(ctx echo.Context, userID string, todoID uuid.UUID, file *multipart.FileHeader) (*todo.TodoAttachment, error) {
	...
	//Verify todo exists and belongs to the user
	...
	//Upload to S3
	s3Key, err := s.awsClient.S3.UploadFile(ctx.Request().Context(), s.server.Config.AWS.UploadBucket, "todos/attachments/"+file.Filename, src)
	if err != nil {
	...
	attachment, err := s.todoRepo.UploadTodoAttachment(ctx.Request().Context(), todoID, userID, s3Key, file.Filename, file.Size, mimeType)
	if err != nil {
...
func (s *TodoService) DeleteTodoAttachment(ctx echo.Context, userID string, todoID uuid.UUID, attachmentID uuid.UUID) error {
	...
	if err != nil {
		logger.Error().Err(err).Msg("failed to fetch attachment details")
		return err
	}

	// Delete attachment record
	err = s.todoRepo.DeleteTodoAttachment(ctx.Request().Context(), todoID, attachmentID)
	if err != nil {
