...
func (h *TodoHandler) UpdateTodo(c echo.Context) error {
	return Handle(
		h.Handler,
		func(c echo.Context, query *todo.UpdateTodoPayload) (*todo.Todo, error) {
			userID := middleware.GetUserID(c)
			return h.todoService.UpdateTodo(c, userID, query)
		},
		http.StatusOK,
...
func (h *TodoHandler) UploadTodoAttachment(c echo.Context) error {
	return Handle(
		...
		func(c echo.Context, payload *todo.UploadTodoAttachmentPayload) (*todo.TodoAttachment, error) {
			...
			if len(files) == 0 {
				return nil, errs.NewBadRequestError("no file uploaded", false, nil, nil, nil)
			}
